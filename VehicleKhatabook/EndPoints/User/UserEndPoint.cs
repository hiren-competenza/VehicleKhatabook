...
namespace VehicleKhatabook.EndPoints.User
{
    public class UserEndPoint : IEndpointDefinition
    {
        ...
        private async Task<IResult> SendOTPforRegisteredUser(ForgotMpinDTO dto, IAuthService authService)
        {
            ...
            string apiUrlWithParams = $"{smsApiUrl}?user=BehraEnterprises&pass=123456&sender=VBEHRA&phone={dto.MobileNumber}&text=Dear User Your OTP For Register to Vehicle Khatabook is {otp.OtpCode} Valid for 10 minutes - BEHRA ENTERPRISES&priority=ndnd&stype=normal";
